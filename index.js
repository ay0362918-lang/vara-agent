...
// --- Find IDL and signerArgs ONCE at startup, never again ---
...
// Pre-build the static parts of the command — only amount changes each call
const BASE_ARGS = [
    ...SIGNER,
    "call", BET_TOKEN,
    "BetToken/Approve",
    "--voucher", null,        // slot 8  — filled in at runtime
...
async function approve() {
    ...
    try {
        ...
        // Truncate error — don't waste time printing huge stack traces
...
async function main() {
    if (!IDL_PATH) {
        console.error("💥 FATAL: bet_token_client.idl not found. Check skills install.");
        process.exit(1);
    }

    log("🔌 IDL found:", IDL_PATH);

    // Set network once — never again
    await execFileAsync("vara-wallet", ["config", "set", "network", "mainnet"], { timeout: 30000 }).catch(() => {});

    await ensureVoucher();
    await registerAgent();

    log("🚀 LOOP STARTED — sequential, zero wasted calls");

    let errors = 0;

    while (true) {
        try {
            // Refresh voucher every 50 successful approves
            ...
            if (ok) {
                ...
                errors++;
                // Only back off after 5 consecutive failures
                if (errors >= 5) {
                    log("⏳ 5 consecutive errors, waiting 2s...");
                    await wait(2000);
                    errors = 0;
                    await ensureVoucher(); // voucher might have expired
                }
            }
